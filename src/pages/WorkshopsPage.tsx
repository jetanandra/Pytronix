...
const WorkshopsPage: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [workshopsData, categoriesData] = await Promise.all([
          categoryId ? getWorkshopsByCategory(categoryId) : getAllWorkshops(),
        ...
        // If we have a category ID, find the selected category
        if (categoryId) {
          const category = categoriesData.find(c => c.id === categoryId);
          setSelectedCategory(category || null);
          setShowAllWorkshops(true); // Show all workshops when a category is selected
        }
      } catch (error) {
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              <a href="#categories" className="btn-primary text-lg px-8 py-3">
                {selectedCategory ? 'View Workshops' : 'Explore Workshops'}
              </a>
            </motion.div>
          </div>
        </div>
      ...
      {/* Featured Workshops */}
      {featuredWorkshops.length > 0 && !selectedCategory && (
      ...
      {/* All Workshops Section - Shown when "View All" is clicked or category is selected */}
      {(showAllWorkshops || selectedCategory) && (
        <section className="py-16 bg-white dark:bg-dark-navy">
          <div className="container-custom">
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-8">
              {selectedCategory ? `${selectedCategory.name} Workshops` : 'All Workshops'}
            </h2>
            ...
            {allWorkshops.length === 0 ? (
              <div className="bg-gray-50 dark:bg-light-navy rounded-lg p-8 text-center">
                ...
                <p className="text-gray-600 dark:text-soft-gray mb-6">
                  {selectedCategory 
                    ? `There are currently no workshops in the ${selectedCategory.name} category.`
